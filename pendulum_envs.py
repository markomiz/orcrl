...
class DoublePendulum(gym.Env):
    ...
    def intu2u(self, u):
        newU = (np.double(u)- np.double(self.nu - 1)/2.0) * np.double(self.max_u * 2 ) / (np.double(self.nu) -1)
        # print(newU)
        return newU
    ...
    def _calculate_reward(self):
        ...
        # A = self.distance(self.q[0], np.pi) ** 2
    ...
    def _update_x(self):
        ...
        self.x[4] = self.l1 * self.q[2]* np.cos( self.q[0])
        self.x[5] = self.l1 * self.q[2]* np.sin(self.q[0])

        self.x[6] = self.x[4] + self.l2 * np.cos(self.q[1]+ self.q[0]) * (self.q[2] + self.q[3])
        self.x[7] = self.x[5] + self.l2 * np.sin(self.q[1]+ self.q[0]) * (self.q[2] + self.q[3])
